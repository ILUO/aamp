...
test('single-agent startup preserves the actionable agent.failed message', () => {
  assert.equal(typeof runtimeNetwork?.agentStartFailureMessage, 'function')
  assert.equal(typeof runtimeNetwork?.preserveAgentStartFailure, 'function')

  const events = [
    {
      type: 'agent.failed',
      agent: 'workbuddy',
      message: 'WorkBuddy is not logged in. Open WorkBuddy and sign in, then retry.',
    },
    { type: 'error', message: 'No agents started successfully' },
  ]

  let process = { events }
  let failure
  try {
    throw runtimeNetwork.preserveAgentStartFailure(
      new Error('ACP Bridge exited before ready'),
      process.events,
    )
  } catch (error) {
    process = undefined
  ...
  assert.equal(
    runtimeNetwork.agentStartFailureMessage(
      failure.agentStartEvents || process?.events,
      'workbuddy',
      failure.message,
    ),
  ...
  assert.equal(
    runtimeNetwork.agentStartFailureMessage([], 'traex', 'bridge exited'),
    'bridge exited',
  )
...
test('install output keeps the final summary without redundant per-binding success lines', () => {
  ...
  assert.match(controller, /title: '已成功建立绑定并启动'/)
  assert.match(controller, /plannedCount: result\.selectedCount/)
  assert.match(controller, /启动成功：/)
  assert.match(controller, /启动失败：/)
