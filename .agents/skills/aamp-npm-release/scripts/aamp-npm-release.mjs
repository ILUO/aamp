...
Options:
  ...
  --agent NAME              Agent used in printed startup commands. Default: trae
...
function parseArgs(argv) {
  ...
  if (!/^[A-Za-z0-9_-]+$/.test(options.agent)) {
    throw new Error('--agent must contain only letters, digits, underscore, or dash')
  }
...
function commandArgsForOptions(options, scope, packageManager, publish) {
  ...
  if (options.agent !== 'trae') args.push('--agent', options.agent)
